...
for CURRENT_NUM_TEST in $(seq 1 "${#FILE_NAMES_EXEC_LIST[@]}"); do
  ...
  if [[ $DEBUG_RECORDS == true ]]
  then
    echo "Starting Test #$CURRENT_NUM_TEST of the Set #$TEST_SET_NUM: ${FILE_NAMES_EXEC_LIST[CURRENT_NUM_TEST - 1]}"
  fi

  rm -f results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  echo -n $'\n' > results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  sed -i "1iTest Files: [ ${FILE_NAMES_EXEC_LIST[CURRENT_NUM_TEST - 1]} ]" results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  sed -i "2i," results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  sed -i "3iNumber of Points, Execution Times in Seconds ($NUM_EXECS Runs/Executions)" results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  sed -i '4i, No. Exec.,,,,, Parallel with Multi-Threading' results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  ...
  sed -i "$ROW_STRING" results_test_files/test_set_0"$TEST_SET_NUM"/test-"$CURRENT_NUM_TEST".csv
  ...
  for NUM_CONTROL_POINTS in $NUM_CONTROL_POINTS_LIST; do
    for NUM_EXEC in $(seq 1 $NUM_EXECS); do
      (time ./energy_storms_seq "$NUM_CONTROL_POINTS" "${FILE_NAMES_EXEC_LIST[CURRENT_NUM_TEST - 1]}") > results_test_files/test_set_0"$TEST_SET_NUM"/temporary_energy_storms_results.csv 2>&1
      ...
      done < results_test_files/test_set_0"$TEST_SET_NUM"/temporary_energy_storms_results.csv
      rm -f results_test_files/test_set_0"$TEST_SET_NUM"/temporary_energy_storms_results.csv
      if [[ $NUM_EXEC -eq 1 ]]
      then
