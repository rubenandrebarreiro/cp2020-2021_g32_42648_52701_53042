...
Cells(paralleWork, locationAvgCol - 1) = "Trabalho sequencial"
...
Dim amdahls As Integer
amdahls = locationAvgRow + (tests * 7) + 14
Cells(amdahls, locationAvgCol - 1) = "Amdahl"
Cells(amdahls + 2, locationAvgCol) = "sequential"
Cells(amdahls + 2, locationAvgCol + 1) = "1 thread"
Cells(amdahls + 2, locationAvgCol + 2) = "2 threads"
Cells(amdahls + 2, locationAvgCol + 3) = "4 threads"
Cells(amdahls + 2, locationAvgCol + 4) = "6 threads"
Cells(amdahls + 2, locationAvgCol + 5) = "8 threads"
Cells(amdahls + 2, locationAvgCol + 6) = "12 threads"
...
For Col = 1 To numberOfColumns - 2
    For Row = 1 To tests
        ...
        Cells(locationSpeedUp + Row, locationAvgCol + Col + 1).Value = speedUp
    Next
Next
...
For Col = 1 To numberOfColumns - 2
    For Row = 1 To tests
        Dim tpp As Double
        tpp = (((List(Col) * Cells(locationNewAvgRow + Row, locationAvgCol + Col + 1).Value) / Cells(locationNewAvgRow + Row, locationAvgCol + 1).Value) - 1) / (List(Col) - 1)
        Cells(paralleWork + Row + 2, locationAvgCol + Col + 1).Value = tpp
    Next
Next
...
For Col = 1 To numberOfColumns - 2
    For Row = 1 To tests
        ...
        serialWork1 = Cells(paralleWork + Row + 2, locationAvgCol + Col + 1).Value
        serialWork2 = 1 - serialWork1
        If serialWork2 < 0 Then
        serialWork2 = 0
        End If

        Cells(serialWork + Row + 2, locationAvgCol + (Col * 3)).Value = serialWork2
        Cells(serialWork + Row + 2, locationAvgCol + (Col * 3) + 1).Value = Cells(paralleWork + Row + 2, locationAvgCol + Col + 1).Value
    Next
Next
...
For Col = 1 To numberOfColumns - 2
    For Row = 1 To tests
        ...
        f = Cells(serialWork + Row + 2, locationAvgCol + (Col * 3)).Value
        t1 = Cells(locationNewAvgRow + Row, locationAvgCol + 1).Value

        result1 = 1 / (f + ((1 - f) / List(Col)))
        Cells(amdahls + Row + 2, locationAvgCol + Col + 1).Value = result1
    Next
Next
